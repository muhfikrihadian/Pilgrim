  ...
  <div class="row">
    ...
    <div class="col l9 s12">
      <div class="row margintopdp">
        <form class="col s12" action="{{ route('admin.recentjobs.search') }}" method="POST">
          {{ csrf_field() }}
          <div class="row">
      ...
      <ul class="collapsible popout" data-collapsible="accordion">
        ...
        @if(isset($jobs))
          @foreach($jobs as $job)
            <li>
              <div class="collapsible-header l2">
                <i class="material-icons">work</i>{{ $job->job_spec }}
                <span class="badge">{{ $job->date_name }}, {{ $job->date }}</span>
              </div>
              <div class="collapsible-body">
                <p>{{ $job->job_desc }}</p>
                <a class="waves-effect waves-light btn" href="{{ route('admin.detail.vacancy', ['id' => $job->id_vacant]) }}">LIHAT</a>
                <a class="red btn modal-trigger right marginleftsp" onClick="deleteVacancy('<?php echo (string)$job->id_vacant ?>')">Hapus</a>
                <a class="green btn modal-trigger right" onClick="$('#confirmpost').modal('open')">Terima</a>              
              </div>
            </li>    
          @endforeach
        @endif
...
<ul class="center pagination">
  ...
  <li class="waves-effect"><a href="#!"><i class="material-icons">chevron_right</i></a></li>
    ...
    <div id="confirmpost" class="modal">
      <div class="modal-content">
        <h5>Apakah Anda yakin akan menghkonfirmasi job ini ?</h5>
        <p></p>
      </div>
      <div class="modal-footer">
